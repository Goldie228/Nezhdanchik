...
RSpec.describe Dish, type: :model do
  ...
  describe "validations" do
    it "is invalid without title" do
      ...
      expect(dish.errors[:title]).to include("не может быть пустым")
    end
    ...
    it "is invalid if cooking_time_minutes is not integer" do
      ...
      expect(dish).not_to be_valid
      expect(dish.errors[:cooking_time_minutes]).to include("должен быть целым числом")
    end
    ...
    it "is invalid if description is too long" do
      ...
      expect(dish).not_to be_valid
      expect(dish.errors[:description]).to include("слишком длинный (максимум 5000 символов)")
    end
  end
  ...
  describe "validations for weight" do
    ...
    it "is invalid with zero weight" do
      ...
      expect(dish).not_to be_valid
      expect(dish.errors[:weight]).to include("должен быть больше 0")
    end
    ...
    it "is invalid with non-integer weight" do
      ...
      expect(dish).not_to be_valid
      expect(dish.errors[:weight]).to include("должен быть целым числом")
    end
    ...
    it "is invalid with too large weight" do
      ...
      expect(dish).not_to be_valid
      expect(dish.errors[:weight]).to include("должен быть меньше 10000")
    end
  end
